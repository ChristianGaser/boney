function [Vo, Yo, Yc, Ya, Ymsk, Ym, Affine, YaROIname, YaROIid, RES, BB] = ...
...
% ##########
    ...
    for ai = 1:numel( job.opts.Patlas )
      if ~isempty(job.opts.Patlas{1})
        ...
        if exist(Pacsv{ai},'file')
          ...
          YaROIname{ai} = [csv(2:end,2)]';
          YaROIid{ai}   = [csv{2:end,1}]';
        else
          YaROIname{ai} = unique(Ya{ai}(:));
          YaROIid{ai}   = YaROIname{ai};
        end
      else
        Ya{ai}        = zeros(size(Ym),'single');
        YaROIname{ai} = 0;
  ...
  %  masked voxels where included (voxel == 1 in the map whereas the face == 2)
  ...
  % limit resolution
  [Yo,RES]    = cat_vol_resize(Yo      ,'reduceV' ,tis.res_vx_vol,job.opts.reslim,16,'meanm');
  Ym          = cat_vol_resize(Ym      ,'reduceV' ,tis.res_vx_vol,job.opts.reslim,16,'meanm');
  for ai = 1:numel(Ya)

...
%   generally not exported) 
  ...
  if isscalar(P)
    ...
    % replace .nii,1 extension
    if strcmp(PC.e(end-1:end),',1')
      if contains(PC.e,'.nii.gz')
        P = spm_file(P,'ext','.nii.gz');
      else
        P = spm_file(P,'ext','.nii');
      end
      [~,PC] = spm_str_manip(P,'tC'); 
    end

    %%
    if numel(PC.s)>0 && PC.s(1)=='m'
      for mi=1:numel(PC.m)
        PC.m{mi} = [PC.s(2:end) PC.m{mi}];
      end
      PC.s = PC.s(1);
    end
    if numel(PC.s)>1 && PC.s(1)=='c' && str2double(PC.s(2))>0 && str2double(PC.s(2))<7
      for mi=1:numel(PC.m)
        PC.m{mi} = [PC.s(3:end) PC.m{mi}];
      end
      PC.s = PC.s(1:2);
    end
  ...
  fmethod = 0; out = [];  
  if ~isempty(PC.s) && ( strcmp(PC.s(1),'m') || ...
      (numel(PC.s)>1 && any(contains({'c1','c2','c3','c4','c5'},PC.s(1:2)))) || ...
      (numel(PC.s)>1 && any(contains({'p0','p1','p2','p3','p4','p5'},PC.s(1:2)))) ) 
    if strcmp(PC.s(1),'m')
    ...
    elseif strcmp(PC.s(1:min(2,size(PC.s))),'c0') && contains(PC.e,'CTseg')  % CTseg
      prefix1 = 3; 
      fmethod = 3;
      job.opts.pmethod = 3; 

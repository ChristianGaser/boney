...
% $Id$
  ...
  def.opts.Patlas       = {fullfile(spm('dir'),'toolbox','boney','boney_KADA_bone-regions-ordered.nii')};
...
if out(i).CTseg || tis.weighting<0, job.affreg = -1; end % this is not optimal here - replace it later
        [Vo,Yo,Yc,Ya,Ymsk,Ym, Affine, YaROIname, YaROIid, RES, BB] = ...
        ...
        [Ybonepp,Ybonethick,Ybonemarrow,Yheadthick, vROI] = ...
          boney_segment_extractbone(Vo,Ym,Yc,Ye,Ya,Ymsk,trans,seg8t,tis,tismri,out(i),job,vx_vol,YaROIname,YaROIid,RES,BB);
        ...
        if job.opts.bmethod>1  
          if  sum(vx_vol <= 2.1)>1  &&  all(vx_vol < 5.1) 
            ...
            [Si, Stm, sROI] = boney_segment_create_bone_surface(Vo, Ybonepp, Ybonemarrow, Ybonethick, Yheadthick, Ya, Ymsk, YaROIname, YaROIid, out(i), job);
          else
            Si = ''; Stm = '';
